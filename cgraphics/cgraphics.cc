...
void CGraphics::DrawAxisAndGrid(IVideo *iVideo_Ptr)
{
 ...
 for(int32_t x=grid_xmin;x<grid_xmax;x++)
 {
  ...
  iVideo_Ptr->DrawLine(screen_x,0,screen_x,cGrRect_Client.Bottom,cGrColor_XGrid.Color,cGrLineStyle_XGrid.Thickness,cGrLineStyle_XGrid.LineType);
 }
 for(int32_t y=grid_ymin;y<grid_ymax;y++)
 {
  ...
  iVideo_Ptr->DrawLine(0,screen_y,cGrRect_Client.Right,screen_y,cGrColor_YGrid.Color,cGrLineStyle_YGrid.Thickness,cGrLineStyle_YGrid.LineType);
 } 
 //рисуем нижнюю границу, отделяющую легенду от графика
 iVideo_Ptr->DrawLine(cGrRect_Client.Left,cGrRect_Client.Bottom,cGrRect_Client.Right,cGrRect_Client.Bottom,cGrColor_XAxis.Color,cGrLineStyle_XAxis.Thickness,cGrLineStyle_XAxis.LineType);
 //рисуем оси
 if (screen_xaxis>=0 && screen_xaxis<cGrRect_Client.Right)
 {
  iVideo_Ptr->DrawLine(screen_xaxis,0,screen_xaxis,cGrRect_Client.Bottom,cGrColor_XAxis.Color,cGrLineStyle_XAxis.Thickness,cGrLineStyle_XAxis.LineType);
 }
 if (screen_yaxis>=0 && screen_yaxis<cGrRect_Client.Bottom)
 {
  iVideo_Ptr->DrawLine(0,screen_yaxis,cGrRect_Client.Right,screen_yaxis,cGrColor_YAxis.Color,cGrLineStyle_YAxis.Thickness,cGrLineStyle_YAxis.LineType);
 }
}
//----------------------------------------------------------------------------------------------------
//отрисовать граифики
...
void CGraphics::DrawGraphics(IVideo *iVideo_Ptr)
{
 ...
 while(current!=end_of_list)
 {
  ...
  if (cGrLineStyle.AsPoint==true)//рисуем как набор точек
  {
   ...
   {
    CGrPoint cGrPoint(cGrDataPtr->GetX(i),cGrDataPtr->GetY(i));
  ...
  else
  {
   for(int32_t i=left_point;i<right_point;i++)
   {
    CGrPoint cGrPoint_1(cGrDataPtr->GetX(i),cGrDataPtr->GetY(i));
    CGrPoint cGrPoint_2(cGrDataPtr->GetX(i+1),cGrDataPtr->GetY(i+1));
	...
	if (cGrPoint_2.X-cGrPoint_1.X>1 && cGrLineStyle.AsStuff==true)//соединяем ступенькой
	{
	 ...
	 int64_t ys1=y1;
	 int64_t ys2=y1;
     GetClipCoord(xs1,ys1,xs2,ys2,0,0,cGrRect_Client.Right,cGrRect_Client.Bottom);
	...
	{
     GetClipCoord(x1,y1,x2,y2,0,0,cGrRect_Client.Right,cGrRect_Client.Bottom);
...
void CGraphics::DrawSelectRectangle(IVideo *iVideo_Ptr)
{
 ...
 {
  iVideo_Ptr->DrawLine(cGrRect_Select.Left,cGrRect_Select.Top,cGrRect_Select.Right,cGrRect_Select.Top,cGrColor_SelectRectangle.Color,1,IVideo::LINE_TYPE_DOT);
  iVideo_Ptr->DrawLine(cGrRect_Select.Right,cGrRect_Select.Top,cGrRect_Select.Right,cGrRect_Select.Bottom,cGrColor_SelectRectangle.Color,1,IVideo::LINE_TYPE_DOT);
  iVideo_Ptr->DrawLine(cGrRect_Select.Left,cGrRect_Select.Bottom,cGrRect_Select.Right,cGrRect_Select.Bottom,cGrColor_SelectRectangle.Color,1,IVideo::LINE_TYPE_DOT);
  iVideo_Ptr->DrawLine(cGrRect_Select.Left,cGrRect_Select.Top,cGrRect_Select.Left,cGrRect_Select.Bottom,cGrColor_SelectRectangle.Color,1,IVideo::LINE_TYPE_DOT);
 }
}
//----------------------------------------------------------------------------------------------------
//отрисовать значения
...
void CGraphics::DrawValue(IVideo *iVideo_Ptr)
{
 ...
 if (EnableXValue==true)
 {
  for(int32_t x=grid_xmin;x<grid_xmax;x++)
  {
   ...
   iVideo_Ptr->PutString(screen_x-static_cast<int32_t>(str_width)/2,cGrRect_Client.Bottom-static_cast<int32_t>(str_height),string,cGrColor_XText.Color);
  }
 }
 //текст по оси Y
 if (EnableYValue==true)
 {
  for(int32_t y=grid_ymin;y<grid_ymax;y++)
  {
   ...
   iVideo_Ptr->PutString(cGrRect_Client.Left,screen_y-static_cast<int32_t>(str_height),string,cGrColor_YText.Color);
  }
 }
}
//----------------------------------------------------------------------------------------------------
...
void CGraphics::DrawLegend(IVideo *iVideo_Ptr)
{
 ...
 while(current!=end_of_list)
 {
  ...
  iVideo_Ptr->GetStringImageSize(name.c_str(),str_width,str_height);
  ...
  //название графика  
  iVideo_Ptr->PutString(x_item_pos,cGrRect_Client.Bottom+LEGEND_Y_OFFSET,name.c_str(),cGrColor_LegendText.Color);
 ...
 //название графика
 iVideo_Ptr->PutString(LEGEND_X_OFFSET,cGrRect_Client.Bottom+7+static_cast<int32_t>(str_height),Name.c_str(),cGrColor_LegendText.Color);
}
//----------------------------------------------------------------------------------------------------
//обрезать линию
//----------------------------------------------------------------------------------------------------
...
{
 size_t amount=cGrData.GetPointAmount();
 left_index=-1;
 right_index=-1;
 if (amount==0) return;//нет точек
 long double left=cGrData.GetX(0);
 long double right=cGrData.GetX(amount-1);
 if (x<=left)
 {
 ...
 while(1)
 {
  ...
  long double xc=cGrData.GetX(center_index);
  if (x<xc)
  {
...
void CGraphics::OnMagnify(void)
{
 ...
 long double stepx=pow(POW_SCALE,floor(log10(grossStepX)));
 ...
 long double stepy=pow(POW_SCALE,floor(log10(grossStepY)));
